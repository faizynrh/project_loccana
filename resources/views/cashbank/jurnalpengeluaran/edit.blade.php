    ...
    <div id="main-content">
        <div class="page-heading">
            ...
            <section class="section">
                <div class="card">
                    <form action="{{ route('jurnal_pengeluaran.update', $data->data[0]->jurnal_parent_id) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                                        ...
                                        <select class="form-select" name="coa_kredit" required>
                                            ...
                                            @foreach ($listcoa as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $data->data[0]->coa_id1 == $item->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control bg-body-secondary" placeholder="Jumlah"
                                            name="credit" id="total_amount" value="{{ $data->data[0]->credit }}" readonly>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label fw-bold">Keterangan</label>
                                        <textarea class="form-control" name="description" placeholder="Keterangan" rows="5" required>{{ $data->data[0]->description_credit }}</textarea>
                                    </div>
                                </div>
                            </div>
                                ...
                                <tbody id="tableBody">
                                    @foreach ($data->data as $item)
                                        <tr style="border-bottom: 2px solid #000;">
                                            <td>
                                                <select class="form-select coa-select" name="items[0][coa_debit]" required>
                                                    <option value="" disabled selected>Pilih Cash Debit</option>
                                                    @foreach ($listcoa as $coa)
                                                        <option value="{{ $coa->id }}"
                                                            {{ $data->data[0]->coa_id2 == $coa->id ? 'selected' : '' }}>
                                            ...
                                            <td>
                                                <input type="number" class="form-control jumlah" placeholder="Jumlah"
                                                    value="{{ $item->debit }}" name="items[0][debit]" required>
                                            </td>
                                            <td>
                                                <textarea class="form-control" placeholder="Keterangan" name="items[0][description]" required>{{ $item->child_description }}</textarea>
                                            </td>
                                        </tr>
                                    @endforeach
        ...
        $(document).ready(function() {
            ...
            const templates = {
                tableRow: (index) => `
                                <tr style="border-bottom: 2px solid #000;">
                                    <td>
                                        <select class="form-select coa-select" name="items[${index}][coa_debit]" required>
                                            <option value="" disabled selected>Pilih Cash Debit</option>
                                            @foreach ($listcoa as $coa)
                                                        <option value="{{ $coa->id }}">
                                                            {{ $coa->account_name }}</option>
                                    ...
                                    <td>
                                        <input type="number" class="form-control jumlah" placeholder="Jumlah" name="items[${index}][debit]" required min="1">
                                    </td>
                                    <td>
